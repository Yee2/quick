...
func netCopy(input, output io.ReadWriter) (err error) {
	buf := make([]byte, 8192)
	for {
...
func handShake(conn io.ReadWriteCloser) (err error) {
	const (
		idVer     = 0
		idNmethod = 1
	)
...
func parseTarget(conn io.Reader) (host string, err error) {
	const (
		idVer   = 0
		idCmd   = 1
...
func pipeWhenClose(conn io.ReadWriteCloser, target string) {
